...
from config import (APP_TITLE, APP_AUTHOR, APP_VERSION, APP_GITHUB,WINDOW_SIZE, WINDOW_MIN_SIZE,LOG_POLL_MS, STATS_POLL_MS, MAX_LOG_LINES,LOG_FONT, DOMAINS_TEXT_HEIGHT, LOG_TEXT_HEIGHT, MODE_COMBO_WIDTH,MODES, DEFAULT_MODE, MODES_SPLIT, DEFAULT_SPLIT, PORT_HTTP, PORT_HTTPS, FAKE_SNI_BASE,TLS_CONTENT_HANDSHAKE, TLS_HANDSHAKE_CLIENT_HELLO, TLS_SNI_EXT_TYPE,TLS_RECORD_HEADER_LEN, TLS_EXT_HEADER_LEN, TLS_MIN_CLIENTHELLO_LEN, SEQ_MASK, SEQ_HALF, CHECKSUM_MASK, CHECKSUM_FLIP, TTL_MIN, TTL_MAX,DOT_BYTE, HTTP_METHODS, HTTP_HEADER_MAX_SCAN, HOST_HEADER, HOST_PORT_SEP, CRLF, CRLFCRLF, Config, CONFIG, DEFAULT_DOMAINS,
)
...
class DpiBypass:
    ...
    def _dispatch_fresh(self, key, packet, payload):
        ...
        if kind == "incomplete" and key is not None:
            with self._pending_lock:
    ...
    def _http_host(payload):
        ...
        while pos < max_scan:
            ...
            if line.lower().startswith(HOST_HEADER):
    ...
    def _detect_host(self, payload):
        ...
        if self._is_http_request_start(payload):
            ...
            if (CRLFCRLF not in payload[:HTTP_HEADER_MAX_SCAN]
                    and len(payload) < HTTP_HEADER_MAX_SCAN):
                return ("incomplete", None)
            return ("none", None)
